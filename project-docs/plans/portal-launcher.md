# Implementation Plan: Portal Launcher
...
**Working repo name:** `Portal-Launcher` (display name: Portal Launcher).  
...
Portal Launcher is a standalone APK. Each widget instance is an independent portal whose destination is selected during widget configuration. Termux Launcher is only one possible destination.

The earlier plan to embed the portal inside the Termux Launcher fork is superseded. The Termux fork may separately receive display-local child-app launching later, but Portal Launcher must not depend on it.
...
Optional interaction: auto-open after a configurable dwell delay, only if the user enables Portal Launcher's AccessibilityService and the visibility-detection experiment succeeds.
...
Portal Launcher app portals should therefore use explicit selected `ComponentName`s plus runtime-resolved cover display options.
...
**enabled Portal Launcher AccessibilityService -> exact Samsung Recents intent -> `ActivityOptions.setLaunchDisplayId(coverDisplayId)`**
...
A newly installed Portal Launcher package may need explicit Good Lock / MultiStar cover-screen authorization before its own Activities are allowed on FlexWindow.

Portal Launcher should also include the relevant Samsung compatibility metadata used by working cover-screen apps:
...
1. User enables Portal Launcher AccessibilityService for Recents.
